...
public interface PlanGroupHistoryDetailRepository extends JpaRepository<PlanGroupHistoryDetail, Long> {
    ...
    @Query(
        ...
        "   ) AS sumOfRecheck " +
        "FROM iso.plan_group_history_detail pghd " +
        "INNER JOIN iso.report r ON r.id = pghd.report_id " +
        "WHERE pghd.result NOT IN ('Đạt', 'PASS') " +
        "  AND r.plan_id = ?1 " +
        "  AND pghd.criterial_name LIKE ?2 " +
        "  AND pghd.criterial_group_name LIKE ?3 ",
        nativeQuery = true
    )
    ...
    @Query(
        ...
        "CONCAT(YEAR(p.time_start), '-', LPAD(MONTH(p.time_start), 2, '0')) as time," +
